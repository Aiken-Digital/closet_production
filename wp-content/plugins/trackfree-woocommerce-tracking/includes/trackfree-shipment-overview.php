            ...
            if ($shipment_details) {
                ...
                if ($total_shipments > 1) {
                    ...
                    <div style="float:right;">
                        <a href="<?php echo get_option('trackfree_tracking_domain') . '/t/' . $tracking_nums; ?>" target="_new"><?php _e('View tracking', 'trackfree-woocommerce-tracking');?></a>
                    </div>
                    <div class="media-router" style="margin-top:10px;">
                        <?php
                        for ($i = 1; $i <= $total_shipments; $i++) {
                            ?>
                            <a href="javascript:void(0);" style="font-size:12px;" class="shipement-menu-item media-menu-item <?php if ($i == 1) { ?> active <?php } ?>" id="shipment-item-link_<?php echo $i;?>">Shipememt <?php echo $i ?></a>
                            <?php
                            }
                        ?>
                    </div>
                    ...
                    if ($trackfree_tracking_number) {
                        ?>
                        <div style="float:right;">
                            <a href="<?php echo get_option('trackfree_tracking_domain') . '/t/' . $trackfree_tracking_number; ?>" target="_new"><?php _e('View tracking', 'trackfree-woocommerce-tracking');?></a>
                        </div>
                        <?php
                        $track_request = array(
                            'key' => $trackfree_account_api_key,
