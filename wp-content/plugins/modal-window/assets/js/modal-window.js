  ...
  $.fn.ModalWindow = function (options) {
    ...
    return this.each(function () {
      var self = this,
        ...
        Click = '1',
        screen = $(window).width();
      $('#' + settings.openIdModalWindow + ', .' + settings.openIdModalWindow + ', a[href$="' + settings.openIdModalWindow + '"]').click(function (event) {
        event.preventDefault();
        if (Click == 1) {
          showModalWindow();
        }
      });
      $('#' + settings.openIdModalWindow + ', .' + settings.openIdModalWindow + ', a[href$="' + settings.openIdModalWindow + '"]').hover(function (event) {
        event.preventDefault();
        if (settings.modalAction == 'hover' && Click == 1) {
          showModalWindow();
        }
      });
      $('#' + settings.closeIdModalWindow + ', #' + settings.closeButtonModalWindow + ', .' + settings.closeIdModalWindow + ', a[href$="' + settings.closeIdModalWindow + '"]').click(function () {
        closeModalWindow();
      });
      $('#' + id).children(".wow-modal-overclose").click(function () {
      ...
      function showModalWindow() {
        ...
        $('#' + id).fadeIn(speed, function () {
          ...
          if (pieces == 'no') {
            $('#' + id).children(".wow-modal-window").fadeIn(speed);
          } else if (pieces == 'fade') {
            $('#' + id).children(".wow-modal-window").show('fade', speed);
          } else {
            switch (pieces[1]) {
              case 'direction':
                $('#' + id).children(".wow-modal-window").show(pieces[0], {
                  direction: pieces[2]
                }, speed);
                break;
              case 'times':
      ...
      function closeModalWindow() {
        ...
        if (pieces == 'no') {
          $('#' + id).children(".wow-modal-window").fadeOut(speed, closeModalOverlay());
        } else if (pieces == 'fade') {
          $('#' + id).children(".wow-modal-window").hide('fade', speed, closeModalOverlay());
        } else {
          switch (pieces[1]) {
            case 'direction':
              $('#' + id).children(".wow-modal-window").hide(pieces[0], {
                direction: pieces[2]
      ...
      function showCloseButton() {
        setTimeout(function () {
          $('#' + id + ' .wow-modal-window').children("#" + settings.closeIdModalWindow).show();
      ...
      function showModalScroll() {
        ...
        $(document).scroll(function () {
          ...
          if ((scrollY > ($(document).height() - $(window).height()) * settings.modalScrollDistance / 100) && !s) {
            s = true;
            delayModalWindow();
          }
        });
      ...
      function modalButtonAnimation() {
        $('.' + settings.buttonId).addClass(settings.buttonAnimationClass + ' wow-infinite ' + settings.buttonAnimation + settings.buttonPosition);
        var animtime = settings.buttonAnimationTime * 1000;
        setTimeout(function () {
          $('.' + settings.buttonId).removeClass(settings.buttonAnimationClass + ' wow-infinite ' + settings.buttonAnimation + settings.buttonPosition);
          modalButtonPause();
        }, animtime)
      }
      ...
      function setModalCookie() {
        ...
        if (days > 0) {
          ...
          document.cookie = settings.cookieName + '=yes; path=/; domain=.' + site + '; expires=' + CookieDate.toGMTString();
        } else if (days === 0) {
