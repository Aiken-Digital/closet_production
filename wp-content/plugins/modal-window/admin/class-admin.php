...
class Wow_Admin_Class
{
  ...
  static function option( $arg ) {
    ...
    elseif ( $type == 'select' ) {
      $disabled = isset( $arg[ 'readonly' ] ) ? ' readonly' : '';
      $option = '';
      foreach ( $options as $key => $value ) {
...
//    wp_enqueue_style( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/css/admin-style.min.css', false, $this->plugin_version );
    ...
    $effects_url = $this->plugin_url . 'assets/js/effects.js';
    wp_enqueue_script( $this->plugin_slug . '-effects', $effects_url, array( 'jquery' ), $this->plugin_version );
...
//    wp_enqueue_script( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/js/admin-script.js', array( 'jquery' ), $this->plugin_version );
    ...
    $send_url = $this->plugin_url . 'assets/js/send.min.js';
    wp_enqueue_script( 'mw-send', $send_url, array( 'jquery' ) );
    wp_localize_script( 'mw-send', 'send_modal_form', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
  ...
  public function rate_us( $footer_text ) {
    ...
    if ( $typenow == $this->plugin_slug ) {
      $rate_text = sprintf( __( 'Thank you for using <a href="%1$s" target="_blank">' . $this->plugin_name . '</a>! Please <a href="%2$s" target="_blank">rate us on WordPress</a>', $this->plugin_pref ), $this->plugin_home_url, 'https://wordpress.org/support/plugin/modal-window/reviews/?filter=5' );
  ...
  function general_admin_notice() {
    echo '<div class="notice notice-warning wow-plugin-notice is-dismissible">
				<p><b>' . $this->plugin_name . '</b>! Please check all your modal windows and make changes if necessary. Please note that the Font Awesome icons and styles of close button have been updated. . <a href="' . admin_url() . 'admin.php?page=' . $this->plugin_slug . '">Go to the ' . $this->plugin_name . ' page</a>. Sorry for the inconvenience.</p>
				</div>';
